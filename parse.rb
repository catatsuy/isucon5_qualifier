...
logs.each do |line|
  path = line[:path].gsub(/^\/profile\/([^\/]+)/, '/profile/:accont_name').gsub(/entry\/([^\/]+)/, 'entry/:entry_id').gsub(/^\/diary\/entries\/([^\/]+)/, '/diary/entries/:account_name').gsub(/^\/diary\/comment\/(\d+)/, '/diary/comment/:entry_id').gsub(/^\/friends\/([^\/]+)/, '/friends/:account_name')
  method = line[:method]
  key = method+'::'+path
  if tmp[key].nil?
