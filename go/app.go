...
func intSliceToCSV(s []int) string {
...
LIMIT 10`, user.ID)
	...
	sort.Ints(friendIds)
	friendIdsCSV := intSliceToCSV(friendIds)
	...
	if *mysqlsock {
		// production
		row := db.QueryRow(myluaQuery(fmt.Sprintf(`{"table":"entries","limit":%d,"offset":%d,"users":[%s]}`, 10, 0, friendIdsCSV)))
		...
		rows, err = db.Query(fmt.Sprintf(`SELECT id, user_id, private, title, created_at FROM entries FORCE INDEX(%s) WHERE user_id IN (%s) ORDER BY created_at DESC LIMIT 10`, entryIndex, friendIdsCSV))
		if err != sql.ErrNoRows {
			checkErr(err)
		}
		for rows.Next() {
	...
	if *mysqlsock {
		...
		row := db.QueryRow(myluaQuery(fmt.Sprintf(`{"table":"comments","limit":%d,"offset":%d,"users":[%s]}`, 10, 0, friendIdsCSV)))
		...
		query := fmt.Sprintf(`SELECT c.id, c.entry_id, c.user_id, c.summary, c.created_at, e.id, e.user_id, e.private, e.title, e.created_at FROM comments c FORCE INDEX (%s) JOIN entries e ON c.entry_id = e.id WHERE c.user_id IN (%s) ORDER BY c.created_at DESC LIMIT 10`, commentIndex, friendIdsCSV)
		// fmt.Println(query)
		rows, err = db.Query(query)
		if err != sql.ErrNoRows {
			checkErr(err)
...
func myluaQuery(args string) string {
	return fmt.Sprintf(`
SELECT mylua('
